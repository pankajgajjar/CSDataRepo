...
class CSUpdateArticle implements ICSUpdateArticle
{
	...
	{
		$updatedProducts = $dataOperations->getAll('select Label,EXTERNALKEY,Language_Code from cs_stg_incr_article where IS_DELETED =' ."'U'"." ".
										'and 
	...
	{
		$updateID = $dataOperations->getAll('Select CS_ATTR_KEY from cs_transaction_map where ATTR_EXTERNALKEY='."'".$externalkey."'");
	...
	{
		$updatedAttributes = $dataOperations->getAll('Select Label,EXTERNALKEY From cs_stg_incr_attribute where IS_DELETED='."'U'"." ".
										'and
	...
	{
		$updatedValues = $dataOperations->getAll('SELECT v.VALUE,v.Language_Code, a.ExternalKey as attrExt,art.EXTERNALKEY as pdExt FROM cs_stg_bridge_artcl_attribute s, cs_stg_incr_attribute a,
	...
	public function updateProductLabels($updatedLabels)
	{
		foreach ($updatedLabels as $label)
		{
			...
			$product->setValue('Label', $label['Label'],$importLanguage->checkIfLanguageExists($label['Language_Code']));
