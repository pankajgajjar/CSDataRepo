...
require_once 'Persitence/CSDataOperations.php';
/**
...
class CSArticleImport implements ICSArticleImport
{
	...
	public function getAllArticles()
	{
		...
		$temp = array();
		$articles = $dataOperation->getAll('Select Label,ExternalKey,Language_Code from cs_stg_incr_article where Language_Code='."'en'" );
	...
	public function getProductAttributes($productID,$language)
	{
		...
		$attributes = $dataOperation->getAll('SELECT a.Label,v.VALUE, a.ExternalKey FROM cs_stg_bridge_artcl_attribute s, cs_stg_incr_attribute a,
	...
	{
		$result = array();
		$languages = $dataOperation->getAll('SELECT DISTINCT Language_Code FROM cs_stg_incr_article');
	...
	{
		$label = array();
		$uniqueLabel = $dataOperation->getAll("SELECT Label FROM cs_stg_incr_article where Language_Code = '".$language."' and  ExternalKey = '".$ExternalKey."'");
	...
	{
		$distinctExternalKey= $dataOperation->getAll('SELECT  Distinct a.EXTERNALKEY a.LABEL FROM cs_stg_bridge_artcl_attribute s, cs_stg_incr_attribute a,
		  cs_stg_incr_attribute_value v,cs_stg_incr_article art, cs_stg_bridge_attributes sb
		    WHERE sb.ATTR_STG_ID = a.ATTR_STG_ID AND sb.ATTR_VALUE_STG_ID = v.ATTR_VALUE_STG_ID  
		    and s.ATTR_STG_BRIDGE_ID = sb.ATTR_STG_BRIDGE_ID  and a.Language_Code = v.Language_Code
		    and s.ARTCL_STG_ID = art.ARTCL_STG_ID
	...
	public function getAttributeLabelwithExternalLanguage($language,$externalKey)
	{
		...
		$attributeLabel = $dataOperation->getAll('SELECT  Distinct a.Label FROM cs_stg_bridge_artcl_attribute s, cs_stg_incr_attribute a,
		  cs_stg_incr_attribute_value v,cs_stg_incr_article art, cs_stg_bridge_attributes sb
		    WHERE sb.ATTR_STG_ID = a.ATTR_STG_ID AND sb.ATTR_VALUE_STG_ID = v.ATTR_VALUE_STG_ID  
		    and s.ATTR_STG_BRIDGE_ID = sb.ATTR_STG_BRIDGE_ID  and a.Language_Code = v.Language_Code
		    and s.ARTCL_STG_ID = art.ARTCL_STG_ID 
	...
	{
		$valueRange = $dataOperation->getAll('SELECT a.label, a.EXTERNALKEY FROM cs_stg_incr_attribute a,cs_stg_incr_attribute_value v,cs_stg_bridge_attributes sb
	...
	public function getDistinctValueRangesValues($attributeid)
	{
		...
		$range = $dataOperation->getAll('SELECT distinct v.VALUE FROM cs_stg_bridge_artcl_attribute s, cs_stg_incr_attribute a,
		  					cs_stg_incr_attribute_value v,cs_stg_incr_article art, cs_stg_bridge_attributes sb
		    				WHERE sb.ATTR_STG_ID = a.ATTR_STG_ID AND sb.ATTR_VALUE_STG_ID = v.ATTR_VALUE_STG_ID  
		    				and s.ATTR_STG_BRIDGE_ID = sb.ATTR_STG_BRIDGE_ID  and a.Language_Code = v.Language_Code 
		    				and s.ARTCL_STG_ID = art.ARTCL_STG_ID 
	...
	{
		$single = $dataOperation->getAll(' SELECT a.label, a.EXTERNALKEY FROM cs_stg_incr_attribute a,cs_stg_incr_attribute_value v,cs_stg_bridge_attributes sb
 								WHERE sb.ATTR_STG_ID = a.ATTR_STG_ID AND sb.ATTR_VALUE_STG_ID = v.ATTR_VALUE_STG_ID AND a.Language_Code = v.Language_Code
      							 AND a.Language_Code = '."'en'".' '.
								'GROUP BY a.LABEL, a.EXTERNALKEY
								HAVING count(*) = 1');
	...
	public function getValueListFromAttributeResult($externalKey)
	{
		...
		$resultValueList = $dataOperation->getAll(' SELECT a.label,v.value,a.EXTERNALKEY
	...
	public function getMultiValuedValuesforProduct($productid,$attributeId){
		...
		$multiValued = $dataOperation->getAll(' SELECT  v.VALUE, a.ExternalKey FROM cs_stg_bridge_artcl_attribute s, cs_stg_incr_attribute a,
		  								cs_stg_incr_attribute_value v,cs_stg_incr_article art, cs_stg_bridge_attributes sb
		   								 WHERE sb.ATTR_STG_ID = a.ATTR_STG_ID AND sb.ATTR_VALUE_STG_ID = v.ATTR_VALUE_STG_ID  
		   								 and s.ATTR_STG_BRIDGE_ID = sb.ATTR_STG_BRIDGE_ID  and a.Language_Code = v.Language_Code 
		   								 and s.ARTCL_STG_ID = art.ARTCL_STG_ID 
	...
	public function getSingleValueAttributesforProduct($externalKey,$language)
	{
		...
		$singleValue = $dataOperation->getAll(' SELECT a.label,v.value,a.EXTERNALKEY
                					FROM cs_stg_bridge_artcl_attribute s, cs_stg_incr_attribute a,
		  					      		cs_stg_incr_attribute_value v,cs_stg_incr_article art, 
                      					cs_stg_bridge_attributes sb
		    						WHERE sb.ATTR_STG_ID = a.ATTR_STG_ID
